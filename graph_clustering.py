...
def graph_clustering(A_matrix, method, n_clusters, ratio=None, graph_num=None, plotting=True, Mean=False):
    ...
    if (Mean):
        graph_num = 0;
        A_matrix = np.mean(A_matrix, axis=0, keepdims=True)
    n = A_matrix.shape[1]
    if (method == 'kmeans'):
        ...
        if (plotting):
            ...
            plt.figure();
            nx.draw(G, node_size=200, pos=nx.spring_layout(G));
            plt.show()
            plt.figure();
            nx.draw(G, node_color=Class, node_size=200, pos=nx.spring_layout(G));
            plt.show()
        return Class
        ...
        clustering = AgglomerativeClustering(n_clusters=n_clusters).fit(
            vecs[:, 1:n_clusters].reshape(-1, n_clusters - 1))
    elif (method == 'Graclus'):
