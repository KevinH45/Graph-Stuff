...
def adj_matrix(train_features_n, test_features_n, win_size, n_sample_train, n_sample_test, n, A_Matrix='cov'):
    if (A_Matrix == 'cov'):
        ...
        if (A_Matrix == 'plv' or A_Matrix == 'iplv'):
            # PLV_Sample
            x_train_plv = np.einsum('ijk,ilk->ijl', np.exp(phase_train * 1j), np.exp(phase_train * -1j)) / (
                        win_size - 1)
            if (A_Matrix == 'iplv'):
            ...
            for i in range(n):
                x_train_pli[:, i, :] = np.abs(
                    np.mean(np.sign(phase_train[:, i, :].reshape(n_sample_train, 1, win_size) - phase_train), axis=2))
            x_train = x_train_pli
            x_test_pli = np.zeros((n_sample_test, n, n))
            for i in range(n):
                x_test_pli[:, i, :] = np.abs(
                    np.mean(np.sign(phase_test[:, i, :].reshape(n_sample_test, 1, win_size) - phase_test), axis=2))
            x_test = x_test_pli
        elif (A_Matrix == 'AEC'):
            # AEC
...
def normalizition(train_features, test_features, normalize, n, win_size):
    if (normalize == 'maxmin'):
        train_features_n = (train_features - np.min(train_features, axis=2, keepdims=1)) / (
                    np.max(train_features, axis=2, keepdims=1) - np.min(train_features, axis=2, keepdims=1))
        train_features_n = 2 * train_features_n - 1
        test_features_n = (test_features - np.min(test_features, axis=2, keepdims=1)) / (
                    np.max(test_features, axis=2, keepdims=1) - np.min(test_features, axis=2, keepdims=1))
...
def preprocess_data(x, Labels, K, Fs, filt=False, ICA=True,
    ...
    if (sampling):
        ...
        train_features = [X_ICA_train[:, :, i: i + win_size] * win for i in
                          range(0, X_ICA_train.shape[2] - step, step)]
        test_features = [X_ICA_test[:, :, i: i + win_size] * win for i in
                         range(0, X_ICA_test.shape[2] - step, step)]
    ...
    train_x, test_x = adj_matrix(train_features_n, test_features_n, win_size, n_sample_train, n_sample_test, n,
                                 A_Matrix)
